...
In the {\ttfamily namespace Optimizers\{...\}}, defined in \textit{Optimizers.hpp}, six classes are defined. Since they have to work either with {\ttfamily W} and {\ttfamily b}, they are defined as class templates. Moreover, in order to be able to use polymorphism, so to let the user choose the desired optimizer runtime, it has been chosen to related they all through inheritance. For this purpose, different design choices were possible: from one side we could simply define a base abstract class ``GeneralOptimizer" from which every optimizer would inherit, ending up with a simple and logically impeccable hierarchy, but with significant code repetitions too; from the other side we could choose a structure like the following:\footnote{``class name = 0'' used as shorthand for ``abstract class''}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/classes0}
	\caption{multiple inheritance pattern}
\end{figure}
\noindent which by sure minimize the repeated code, but introduces significant complications in the code, such as the comparison of  deadlock diamonds structures and the consequent need of virtual inheritance.\\
At the end the choice has been to design a hierarchy with this objective in mind, in order of priority:
\begin{enumerate}
...
\end{enumerate}
so in short, to write something which is functional and minimal, in case at the expense of a strict compliance to the standard paradigm. For this reason gradIN GENERAL, preferito approccio da particolare a generale, perché più vicino al percorso naturale di sviluppo--> spiega. Aggiungi anche nota in NeuralNet che rimanda qui, al 3.3., che dice: unique\_ptr a grad\_desc perchè --> vd questo capitolo.
\begin{figure}[H]
	\centering
