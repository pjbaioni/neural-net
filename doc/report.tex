...
In chapter \ref{chapter3} we explain the programming architectural choices that have been taken during the development of the code and we illustrate its structure and some of the content of its main files, also providing some insights with respect to the actual implementation of the previously stated theory and methods.\\
In chapter \ref{chapter4} instructions on how to obtain\footnote{The code is freely available on GitHub: \href{https://github.com/pjbaioni/neural-net}{\emph{https://github.com/pjbaioni/neural-net}}}, compile, link and run the program are given, as well as a complete benchmark case, which serve both as test example to verify the correct functioning of the code and also as opportunity to generalize a little bit and speak about the possible behavior of the neural net when settings are changed.\\
...
\end{lstlisting}
As evident, the code strongly relies on the Eigen built-in operations and function; in particular at line 7 and 11 of the above code a Python-like \textit{broadcasting} is implemented. Another great advantage of using Eigen is the ease of switching between matrix like operations, like the products in the same lines of code, to element wise operations, invoked with a {\ttfamily .array()}; moreover, when optimization is activated, e.g. with the {\ttfamily g++ -O3} option, such conversions, as well as transpositions and so on, are implemented so that they are costless from a computational point of view.\\
...
%This steps results in an output like the following:\\
%{\\ \ttfamily 
%	\$ ./main.out\\
...
In chapters \ref{chapter3} and \ref{chapter4} we have focused on explaining how to develop from scratch a DNN based efficient and numerical analysis relevant application, exposing both the technicalities of the adopted algorithms and the design choices that have been taken, both in the details of the implementation and in the general architecture of the program. After this a complete benchmark case  has been presented, accompanied by comprehensive instructions on how to build and run the program.\\
Before concluding, we notice that it isn't uncommon to find comparisons between results achievable with DNNs metods and FEMs, e.g. \cite{Jinchao} prove that in a 1D specific example a ReLU DNN can recover a better result, in terms of convergence accuracy, than adaptive finite element method. In such comparisons it's worth considering that the time required by a DNN-like approach is very different from the one of standard methods, indeed from one side tuning a DNN program properly requires a great amount of time, because even hyperparameters which have given great result in one field of application are often completely not suitable to the others, so a large number of combinations have to be tried; from the other side, once tuned and trained, the DNN is ready perform different test, and so to make all the predictions needed, just at the price of one forward propagation, while the FEM solver has to work every time from scratch.\\ 
