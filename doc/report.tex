...
The choices made have thus led to a structure as the following one:\footnote{The class holds vectors of pointers to Gradient Descent in order to implement polymorphism, see section \ref{section3.3} for further insights.}
...
In the {\ttfamily namespace Optimizers\{...\}}, defined in \textit{Optimizers.hpp}, six classes are defined. Since they have to work either with {\ttfamily W} and {\ttfamily b}, they are defined as class templates.\\ 
Moreover, in order to be able to use polymorphism, and so to let the user choose the desired optimizer runtime, it has been chosen to relate them all through inheritance. For this purpose, different design choices were possible: from one side we could simply define a base abstract class ``GeneralOptimizer" from which every optimizer would inherit, ending up with a simple and logically impeccable hierarchy (every optimizer \textit{is a specialization of} the general optimizer), but with significant code repetitions too; from the opposite one we could choose a structure like the following:\footnote{``class name = 0'' used as shorthand for ``abstract class''}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/classes0}
	\caption{multiple inheritance pattern}
\end{figure}
\noindent which by sure minimizes repetitions, but introduces significant complications in the code, such as the comparison of  deadlock diamonds structures and the consequent need of virtual inheritance.\\
In order to take the decision it has been given a different priority to the objectives and constraints:
\begin{enumerate}
	\item guarantee polymorphism;
	\item avoid multiple inheritance;
	\item minimize code repetition;
	\item minimize classes number;
\end{enumerate}
So in short, it has been decided to write something which is functional and minimal, in case at the expense of a strict compliance to the standard paradigm. In fact, it has been preferred a bottom-up approach, starting from the direct implementation of a gradient descent step into the NeuralNetwork train() function, then moving to a separate class callable object, and then ``adding pieces" which have been found to be interesting or useful. Moreover, implementing the code, it turned out that gradient descent is actually the most basic class one could imagine for a working optimizer. For these reasons and having this picture in mind, it has been chosen as base class; more generally the children classes in the following Figure \ref{fig10} inherit from their father in the sense that they start from it and then add their own functionalities and particularities:
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{img/classes1}
	\caption{single inheritance pattern}\label{fig10}
\end{figure}
\noindent Finally, it has been chosen to give the class {\ttfamily Adam<T>} an {\ttfamily RMSprop<T>} member, so using an \textit{has a} approach, even if only in this case. This has the cons of treating in a not-symmetric way the two steps, gradient descent with momentum and RMSprop, which together build up the Adam algorithm, but has the pros of avoiding multiple virtual inheritance in a simple and effective way. In order to keep the correct encapsulation in the {\ttfamily RMSprop<T>} class while guaranteeing {\ttfamily Adam<T>} class had the necessary access to the members of its {\ttfamily RMSprop<T>} member {\ttfamily rms}, class {\ttfamily RMSprop<T>} privately declares class {\ttfamily Adam<T>} as friend.\\
...
\end{lstlisting}
The {\ttfamily assert} at line 8 could have been placed equivalently outside the {\ttfamily if} block, since {\ttfamily assert} automatically checks whether {\ttfamily NDEBUG} is defined or not, however it has been included to improve readability. 
\newpage
...
\newpage
\section{The main}
The {\ttfamily main.cpp} source file takes into account for both execution of the main program and input/output operations. The input stage is managed analogously to {\ttfamily write\_set.cpp}, thus it's not reported here; for example we have:\\
{\small \ttfamily 
	\$ ./main.out -h\\
...
assert(read);
\end{lstlisting}
Since the reading has to work both with matrices and vectors, {\ttfamily read\_set(...)} has been implemented as a function template, so to work with any matrix-like type which overloads of the call operator for accessing its elements:
\begin{lstlisting}[frame=single, showstringspaces=false]
template <typename T> 
...
\end{lstlisting}
After that, the network is constructed following the ``architectureNL.dat" file instructions, where NL is the number of layers, calling the constructor of the NeuralNetwork class, and then trained and tested:
\begin{lstlisting}[frame=single]
//Construct the net:
...
Differently, the test results are printed later, directly by the main, both in a written and graphical way. \\
First the numerical results are output on the terminal window and the predicted solution is saved in the prevision file, that is \$data/Yhat*.dat:
\begin{lstlisting}[frame=single, showstringspaces=false]
//Print the results:
...
//plot:
xspacing == 0 ?
gp<<"plot"<<gp.file1d(std::tie(xtest,ytest))<<"w lp lw 4 title 'Test Data',"<< gp.file1d(std::tie(xtest,prevision))<<"w lp lw 1.5 title 'Prevision'"<<endl :
gp<<"plot"<<gp.file1d(std::tie(xtest,ytest))<<"pt 7 title 'Test Data',"<< gp.file1d(std::tie(xtest,prevision))<<"pt 3 title 'Prevision'"<<endl;
\end{lstlisting}
The results are plotted with line and points if sorted, while only with points if random generated. \\
It's also possible to plot in a second time this last graph without executing again the main program. Indeed, the data folder contains the ``PlotPrevisionWithLines.gnu" and ``PlotPrevisionWithPoints.gnu" gnuplot scripts too, which make use of the {\ttfamily paste}\footnote{ see e.g. {\ttfamily \$ man paste}} GNU coreutil to avoid duplicates of the common x-coordinate, e.g.:\footnote{At the moment of writing, some graphical functionalities, like adding the grid interactively from the plot window, are possible only when the scripts is used, due to current version gnuplot-iostream limitations.}
\begin{lstlisting}[frame=single, showstringspaces=false]
...
\end{lstlisting}
and which can be called typing simply:\\
{\small \ttfamily data \$ gnuplot PlotPrevisionWithLines.gnu\\}
or the analogous with points.
...
solve poisson2d(uh,vh,solver=CG)=
