...
Unfortunately this situation is very common in DNN programming. Some general methods to face it are provided in chapter \ref{chapter1}, while some practical advices related to the present code are given in the next \ref{section4.3} section. 
\section{Main commands}\label{section4.3}
make run con esempio funzionamento, + esempio più preciso di cambiare i refinements etc (meglio solo runtime changes), comandi generali and local con OPTIONS+=, + ./main.out --help e idem per write\_set
\section*{Old section}
It's also possible to pass options to the main by make, redefining the variable {\ttfamily OPTIONS} runtime, e.g.:\\
{\\ \ttfamily 
