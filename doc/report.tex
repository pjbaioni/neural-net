...
It is worth noting that, especially in DNN programming, the more the code is enriched and exponentially the more tuning is necessary; in fact, not only the new choices must be tested, but also all their combinations with the previous hyper-parameters.
It's worth considering that, especially in DNN programming, the more the code is enriched and exponentially the more tuning is necessary; indeed not only the new choices have to be tested, but also their combinations with the already existing hyper-parameters have so. It's thus clear that at the moment that would go over the aims of the present project as they have been outlined in the \hyperref[introduction]{introduction}.
...
The {\ttfamily KingmaBa<T>} class is the only abstract class of the hierarchy. It has been introduced to provide the common interface to two great optimizers, Adam and AdaMax, which have been introduced in the same article by \cite{Kingma}:
\begin{lstlisting}[frame=single]
template<typename T>
...
\end{lstlisting}
\noindent The Adam optimizer includes an elevation to the iteration number-th power, which is of unsigned type. Since {\ttfamily std::pow()} is known to be a ``performance killer", because it solves a non linear problem which is completely unnecessary when the exponent is an integer, at first the {\ttfamily Adam<T>} class used a function like this one:
\begin{lstlisting}[frame=single]
...
\end{lstlisting}
But later on, compiling with optimization, it turned out that, on the performed test, the standard library function was actually faster, with a total execution time of 5-6 seconds instead of 14-15 seconds on the used machine, and guaranteed a better result too, namely $~0.7 \%$ instead of $~1.1\%\,\,L_2$ relative error on the test set.\footnote{This seems to be due to the fact that the iteration number, that is the exponent, becomes early a large number, typical values ranging from $10^4$ to $10^5$.} So the Adam optimizer has been implemented in this way:
...
\end{lstlisting}
The above implementation doesn't follow strictly the one proposed in the literature: the correction step required by the algorithms has been composed with the update step, for performance reason. Anyway, also the standard algorithm steps are present in the code, albeit commented out (not reported here to save space).\\
